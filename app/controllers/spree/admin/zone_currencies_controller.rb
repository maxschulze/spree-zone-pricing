...
class Spree::Admin::ZoneCurrenciesController < Spree::Admin::BaseController 
	...
	def index
		...
		zones.each do |zone|
			zone_currency = Spree::ZoneCurrency.find_by_zone_id(zone.id)
			if zone_currency.nil?
				zone_currency = Spree::ZoneCurrency.new
				zone_currency.zone_id  = zone.id
				zone_currency.currency = Spree::CurrencyPrice.currency_types[0]
				zone_currency.save
			end
		end

		@zone_currencies = Spree::ZoneCurrency.page(params[:page]||1).per(20)
	end
	...
	def create
		@zone_currency = Spree::ZoneCurrency.new(params[:zone_currency])
	...
	def edit
		@zone_currency = Spree::ZoneCurrency.find(params[:id])
